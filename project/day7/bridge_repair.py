...
def valid(test_value, equation, operators):
    possible_sums = reduce(
        lambda acc, value:
        filter(lambda current_sum: current_sum <= test_value,
               flatten(map(lambda previous_sum: [operator(previous_sum, value) for operator in operators], acc))),
        equation[1:],
        [equation[0]])
...
if __name__ == "__main__":
    with open("input", "r") as file:
        ...
        equations = [(int(line.split(':')[0].strip()), list(map(int, line.split(':')[1].strip().split(' '))))
                     for line in lines]
