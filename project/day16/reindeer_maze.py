...
def run(grid):
    ...
    paths = {(start[0], start[1], 1): [(start[0], start[1], 1)]}
    while to_visit:
        ...
        if grid[in_front[0]][in_front[1]] != '#' and (in_front not in scores or scores[in_front] > scores[current_pos] + 1):
            ...
            paths[in_front] = paths[current_pos] + [in_front]
        for rotation in [1, -1]:
            ...
            if new_pos not in scores or scores[new_pos] > scores[current_pos] + 1000:
                ...
                paths[new_pos] = paths[current_pos] + [new_pos]
    ...
    return score, set(map(lambda pos_rot: (pos_rot[0], pos_rot[1]), paths[endpoint]))


if __name__ == "__main__":
    with open("test1", "r", newline='\n') as file:
